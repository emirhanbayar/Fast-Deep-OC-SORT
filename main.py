...
def get_main_args():
    ...
    parser.add_argument("--occlusion_threshold", type=float, default=0.2)
    parser.add_argument("--aspect_ratio_threshold", type=float, default=0.65)
...
def main():
    ...
    for (img, np_img), label, info, idx in loader:
        ...
        cv2_frame = cv2.imread(os.path.join("datasets/mot/train", info[4][0]))

        targets = tracker.update(pred, img, np_img[0].numpy(), tag, cv2_frame)
        tlwhs, ids = utils.filter_targets(targets, args.aspect_ratio_thresh, args.min_box_area)

        # cv2 write ids on boxes
        for tlwh, id in zip(tlwhs, ids):
            x1, y1, w, h = tlwh
            # cv2.rectangle(cv2_frame, (x1, y1), (x1 + w, y1 + h), (0, 255, 0), 2)
            x1, y1, id = int(x1), int(y1), int(id)
            cv2.putText(
                cv2_frame,
                str(id),
                (x1, y1),
        ...
        os.makedirs(f"./debug/{video_name}", exist_ok=True)
        cv2.imwrite(f"./debug/{video_name}/{frame_id}.png", cv2_frame)
...
def draw(name, pred, i):
    pred = pred.cpu().numpy()
    name = os.path.join("datasets/mot/train", name)
    img = cv2.imread(name)
    for s in pred:
